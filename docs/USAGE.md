...
**관리자** — `/quantumpets` 는 `/qpt` 로 줄여 써도 됩니다.
...
펫이 다음 형태로 자라는(성장도가 다 차는) 순간, 그동안 쌓인 포만도가 충분하면
다음 형태 대신 돼지가 됩니다.
...
gimmick:
  overfeed:
    enabled: true
    min-fullness: 0      # 진화하는 순간의 포만도가 이 값 이상이어야 함 (0 이면 조건 없음)
    chance: 100          # 위 조건을 채워도 이 확률(%)로만 발동
    becomes: pig         # 이 종류로 바뀝니다 (pets/ 에 있는 id)
    # model: pet_pig     # 종류는 그대로 두고 모습만 이걸로 바꿉니다 (becomes 와 별개)
```

- **next-stage 로 진화할 수 있는 종류만** 대상입니다. 진화할 곳이 없는(next-stage
  가 없는) 종류는 애초에 이 순간이 오지 않으므로 아무리 먹여도 돼지가 되지 않습니다
...
- `model` 은 **종류는 그대로 두고 모습만** 바꿉니다 — 별도의 `pets/*.yml` 없이 겉모습만
  ...
  `model` 을 따릅니다
- 조건을 못 채우면 **원래대로 다음 형태로 정상 진화합니다.** 끄고 싶으면
  `enabled: false` 를 쓰세요
