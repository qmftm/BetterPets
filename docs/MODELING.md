...
| 이름 | 필수 | 재생 | 언제 걸리나 |
| --- | :---: | --- | --- |
| `idle` | ✅ | 반복 | 주인 곁에 서 있을 때 |
| `walk` | ✅ | 반복 | 따라 걸을 때 |
| `run` | ✅ | 반복 | 8블록 이상 뒤처져 뛸 때 |
| `ride` | 탑승 펫 | 반복 | 지상 탑승 중 |
| `fly` | 비행 펫 | 반복 | 비행 탑승 중 |
| `eat` | | 1회 | 먹이를 먹을 때 (다른 동작 위에 얹힌다) |

**여기 없는 이름은 만들어도 지금은 재생되지 않아요.** 만들다 만 기능에 시간을 쓰지
않으시도록 적어둡니다 — 아래 넷은 자리만 잡아둔 상태입니다.

| 이름 | 언젠가 쓸 곳 |
| --- | --- |
| `fly_idle` | 공중 정지(호버링). 지금은 비행 탑승 전체가 `fly` 하나로 간다 |
| `sit` | 대기 명령. 명령 자체가 아직 없다 |
| `attack` | 전투 동작. 아직 어떤 전투 기능도 없다 |
| `spawn` | 소환 연출 |

**없는 애니메이션은 `idle` 로 접습니다.** 비행 펫에 `fly` 를 안 만들었으면 나는 동안
정지 자세로 보여요 — 어색하지만 아무것도 안 도는 것보다는 낫습니다. 다만 `idle` 마저
없으면 그 펫은 정말로 멈춰 있습니다.

> 플러그인은 모델 안에 어떤 애니메이션이 들었는지 **미리 알지 못해요.** 재생을 시도해
> 보고 실패하면 그때 접습니다. 그래서 `.yml` 을 읽는 시점에는 경고가 나오지 않습니다.
...
기본 설정이 `pet_pig` 모델을 참조한다. next-stage 로 진화하는 순간 포만도가 충분하면
이 종류로 바뀌므로, **이 모델이 없으면 변신했을 때 소환이 실패한다.** 걷는 탑승만
되는 평범한 지상 펫이면 된다.
