...
`EggDefinition`/`Weighted` 와 같은 결의 무작위성). **시간이 지나도 저절로 안 내려간다** —
한 번 상한에 닿은 펫은 그 개체 수명 내내 못 먹인다. 그래도 성장이 완전히 막히는 건
아니다: 시간 경과(1분당 +1)는 포만도와 무관하게 그대로 돌아서, 급여로는 못 채워도
기다리면 결국 다 자란다. 과급식 기믹(아래)과는 판정 축이 다르다 — 과급식은 **짧은
시간 안의 횟수**를 보고, 포만도는 **누적값**을 본다. 같은 "너무 많이 먹였다"는 상황을
막는 셈이지만 서로 독립적으로 판정한다.
