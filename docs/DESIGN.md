...
**탑승도 능력도 생애주기와 무관하게 처음부터 된다 — `PIG` 만 예외다.** 원래는
`ADULT` 여야 능력이 붙었는데("다 자라야 쓸모 있어진다"), 그 대기가 모든 펫에게
있을 이유가 없다는 요청으로 뗐다. `BABY`/`ADULT` 를 가르는 건 이제 순전히
`next-stage` 가 있는 펫(예: `hatchling`)이 진화해 다른 종류가 되는가뿐이고,
`PIG` 만 과급식의 대가로 능력을 잃는다.
...
`decay-seconds` 를 0 이하로 두면 꺼진다. 그래도 성장이 완전히 막히는 건 아니다: 시간
경과(1분당 +1)는 포만도와 무관하게 그대로 돌아서, 급여로는 못 채워도 기다리면 결국 다
자란다. 과급식 기믹(아래)과는 판정 축이 다르다 — 과급식은 **짧은 시간 안의 횟수**를 보고,
...
탑승 속도(`ride-speed`)도 같은 자리 방식이다 — `rarity.yml` 의 등급별 값이 기본값이고, `pets/*.yml` 에 `ride-speed` 를 적으면 그 펫만 다른 속도를 쓴다. `PetType` 에 `flyChance` 와 나란히 필드를 뒀다("이 펫만 다르게" 오버라이드 전부 같은 자리에 두는 게 찾기 쉽다).
...
생애주기 이름도 마찬가지다. `LifeStage.name()` 을 그대로 쓰면 한국어 화면에 `BABY` 가 튀어나온다. `stage.<이름 소문자>` 키로 찾는다.
...
| 시간이 흘러 자람 | `GrowthCatchUp.announce` |

후자를 빠뜨리면 "가만히 뒀더니 조용히 성체가 됐다"가 된다. 성장도가 시간 경과로도 오르는 설계라 이 경로가 오히려 더 흔하다.

**주인에게는 방송 문턱과 무관하게 알린다.** 방송은 A등급부터 나가는데 그것만 두면 D등급 펫은 정말로 조용히 성체가 된다. 방송의 목적은 자랑이고 개인 알림의 목적은 "네 펫이 달라졌다"라 문턱이 같을 이유가 없다.

### 성장은 계산이지만 성체가 되는 것은 사건이다

성장도는 `updated_at` 기준 지연 계산이라 읽는 순간 저절로 맞는다. 그런데 **아기 → 성체 전이는 누가 확인해 줘야 일어난다.** 확인하는 자리가 틱 루프뿐이었고 틱은 소환된 펫만 돈다. 결과:

- 보관함에 넣어둔 펫은 성장도가 상한에 붙은 채 아기로 굳었다. 상한에 붙으면 `refresh` 가 값을 바꾸지 않으니 **꺼내서 틱을 태워도 영원히 그대로였다** (틱이 "값이 바뀌었을 때만" 확인했다)
- 접속하지 않은 동안 자란 펫도 마찬가지다. 문서는 "접속해 있지 않아도 자란다"고 적어놓고 있었으니 설명과 동작이 갈린 자리였다
...
- [ ] 아기 펫도 소환 즉시 능력이 붙어 있는가. 과급식으로 돼지가 되면 그때만 떨어지는가
