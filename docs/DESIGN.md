...
| 항목 | 원작 | 본 설계 | 이유 |
| --- | --- | --- | --- |
| 등급 | D~S, 등급별 이동속도 배율 | D~S, **표기 전용(속도 배율 없음)** | 등급이 실력 차이가 아니라 이름표 하나로만 쓰이도록 요청받았다 |
...
이들이 릴리스 노트에서 고쳤다고 밝힌 것들이 곧 우리가 만날 버그다. **무엇을 실제로
가져왔는지까지 적는다** — "가져올 것" 목록만 두면 언젠가 "가져왔다"로 읽힌다.

| 문제 | 해법 | 지금 |
| --- | --- | --- |
| 빠른 마운트가 얇은 벽을 **관통** | 이동 경로를 **0.45블록 이하 서브스텝으로 분할 검사**. 하나라도 막히면 전체 취소 | ✅ `RideController.tryMove` |
| 벽에 닿으면 이동이 완전히 멈춤 | **벽 슬라이딩** — 전체 → 수평만 → 수직만 순으로 시도 | ✅ `RideController.drive` |
| 어깨/머리가 벽에 **끼임** | 중심 + 반경 0.35의 4방향 점을 **발치와 머리 높이 양쪽에서** 검사 | ✅ `RideController.isSafe` |
| 실수로 이륙 | **두 번째 우클릭 확인**을 요구 | ✅ `InteractionListener.tryRide` |
| 비행 중 우클릭하면 하차돼버림 | 비행 중 하차는 **`/pet dismount` 전용**(스니크는 하강) | ✅ |

> **탑승이 아닌 일반 추종 이동에도 같은 문제가 있었다.** `MovementController`(주인을
> 따라다니는 이동)는 이 서브스텝 기법을 처음엔 안 가져왔는데, 실기에서 펫이 벽을
> 뚫고 오고 절벽 위에서 허공에 뜬 채로 미끄러지는 게 확인됐다. 원인은 `groundLevel`
> 이 "여기는 못 지나간다"(2칸 벽, 3칸보다 깊은 낙차)는 뜻으로 `currentY` 를 그대로
> 돌려주는데, 호출부가 그걸 실패로 못 알아듣고 x·z 는 그대로 이동시킨 것이었다. 위
> 표의 첫 줄과 같은 기법(서브스텝 분할 검사)을 `MovementController.advance` 에도
> 적용해서 막힌 지점 직전까지만 이동하게 고쳤다.
| 고도 제한 | 빌드 높이가 아니라 `flight-max-height`(기본 1024)로 별도 상한 | ✅ |
| 비행 중 히트박스가 **블록 파괴 레이캐스트를 가로챔** | 비행 중 0.1×0.1로 축소, 하차 시 복원 | ❌ 그쪽은 히트박스가 별도 `Interaction` 엔티티라 따로 줄일 수 있다. 우리는 캐리어 `Mob` 자체가 히트박스라 줄이려면 `SCALE` 을 건드려야 하고, 그러면 그 위의 모델까지 같이 줄어들 수 있다. 서버 없이 확인할 수 없어 손대지 않았다 |
| 남이 펫을 우클릭해 아이템을 잃음 | 캐리어(`Allay`/`Rabbit`)·마운트(`ArmorStand`) 모두 <b>바닐라 상호작용이 손에 든 것을 받아 간다</b>(알레이는 그대로 받고, 토끼는 번식용 먹이를 먹는다). 상호작용은 소유자 확인보다 <b>먼저</b> 취소하고, 마운트는 슬롯을 통째로 잠근다 | ✅ 우리가 찾은 것 — 오프핸드와 "남의 펫"이 실제 구멍이었다 |
| 펫이 채굴·건축을 방해 | 우클릭 동작이 없는 펫은 히트박스를 아예 작게 | ❌ 위와 같은 이유. 대신 캐리어를 **`Allay`**(0.35×0.6)·**`Rabbit`**(0.4×0.5)로 골라 애초에 작게 뒀다 |
| 펫 본체가 마운트를 못 따라감 | `setTeleportDuration(1)` — 탑승 중 1틱 보간, 평상시 8틱 | ❌ **쓸 수 없다.** `setTeleportDuration` 은 `Display` 에만 있고 `Entity`/`Mob` 에는 없다 (paper-api 26.2 확인). 그쪽은 본체가 `ItemDisplay` 라서 되는 것이다. 우리 캐리어는 `Mob` 이라 매 틱 `teleport` 로 붙인다 |

> **하차 시 낙하 피해 방지는 뺐다.** 처음엔 표대로 `setFallDistance(0)` + `SLOW_FALLING`
> 100틱을 걸었는데, 공중에서 내려도 안 떨어지는 게 오히려 어색하다는 요청으로
> `RideController.stop` 에서 걷어냈다. 지금은 `setFallDistance(0)` 만 남아 있다 —
> 탑승 전에 이미 떨어지던 중이었을 낙하 거리만 하차 시점에서 다시 세게 하려는
> 것이지, 낙하 피해 자체를 막지는 않는다. 비행 중 `/pet dismount` 로 높은 곳에서
> 내리면 그대로 떨어진다.
...
**꺼내져 있는(`active`) 펫만 시간으로 자란다.** 보관함에 넣어둔 동안은 아무리 시간이
지나도 성장도가 안 오른다 — `GrowthService.growsOverTime` 이 `data.active()` 를
먼저 본다. 자라지 않는 동안에도 기준 시각(`updatedAt`)은 지금으로 계속 당겨진다 —
안 그러면 나중에 다시 꺼냈을 때 보관함에 머문 시간이 한꺼번에 성장도로 잡힌다.

### 포만도 — 먹여서 크는 속도에 상한을 둔다

성장도와 별개인 값이다. `GrowthService.feed()` 가 성장도를 올리기 **전에** 먼저 본다 —
`growth.fullness.max` 이상이면 성장도도 포만도도 건드리지 않고 `TOO_FULL` 을 돌려준다.
호출부(`InteractionListener.feed`)는 이 결과를 받으면 **아이템을 소비하지 않는다** — 보유
한도가 찼을 때 알을 안 까는 것과 같은 규칙이다.

통과하면 `growth.fullness.min-gain`~`max-gain` 사이에서 무작위로 골라 더한다(`ThreadLocalRandom`,
`EggDefinition`/`Weighted` 와 같은 결의 무작위성). `growth.fullness.decay-seconds` 마다
1씩 저절로 내려간다 — `GrowthCurve` 와 같은 지연 계산 패턴을 반대 방향으로 쓴 `FullnessCurve`
가 맡는다. 성장도의 기준 시각(`updatedAt`)과 섞이지 않게 `fullnessUpdatedAt` 을 따로 둔다:
급여(`addFullness`)는 값만 올리고 이 시각은 건드리지 않는다 — 건드리면 감소분을 한 번
반영해 둔 직후 또 먹였을 때 그 사이 시간이 다시 감소 몫으로 잡히는 이중 계산이 된다.
`decay-seconds` 를 0 이하로 두면 꺼진다. 포만도가 상한에 닿아도 성장이 완전히 막히는
건 아니다: `next-stage` 가 있는 종류라면 시간 경과(꺼내둔 동안 1분당 +1)는 포만도와
무관하게 그대로 돌아서, 급여로는 못 채워도 기다리면 결국 진화한다(`next-stage` 가
없는 종류는 애초에 급여로도 시간으로도 안 자란다 — 위 "성장도의 유일한 역할" 절 참고).

과급식 이스터에그(아래)는 바로 이 누적 포만도를 본다 — 짧은 시간의 횟수가 아니라
**진화하는 순간의 포만도**다.

### 성장도의 유일한 역할 — 다음 종류로 진화하기까지 걸리는 시간

원작에는 없던 확장이자, 이 프로젝트에서 성장도가 하는 일의 전부다. 능력치
스케일링도, 비행 여부 추첨도, 성체 전이도 예전에는 성장도가 관여했지만
전부 걷어냈다 — "성장도는 다음 펫으로 진화하는 용도 하나뿐"이라는 요청에
맞춰서다.
...
0. **먼저 돼지가 될지 본다.** `gimmick.overfeed.enabled` 가 켜져 있고, 그 순간의
   포만도가 `min-fullness` 이상이며 `chance` 를 통과하면, 다음 형태로 넘어가는 대신
   `LifeStage.PIG` 로 빠진다(아래 생애주기 표). 그러지 못했으면 아래 순서로 정상 진화한다
...
뽑힌 종류에마저 `next-stage` 가 없으면, 그 뒤로는 위 첫 문단으로 떨어져 성장도가
멈춘다. **전역 "최대 단계" 설정은 없다** — 사슬이 끝나는 지점은 순전히 각 펫
파일의 `next-stage` 유무로 정해진다. 같은 이유로 `next-stage` 가 없는 종류는
0번의 진화 순간 자체가 오지 않으므로 **아무리 먹여도 돼지가 되지 않는다.**

```yaml
# pets/baby_phantom.yml
next-stage:
  phantom_normal: 70   # 70% 확률로 평범한 팬텀 (next-stage 없음 — 여기서 사슬이 끝난다)
  phantom_ender: 30    # 30% 확률로 엔더 팬텀 (역시 끝)
```
...
소환 지점은 **소유자 뒤 1.5블록**이 기본이지만, 거기가 벽이나 바닥 속일 수 있다. 캐리어가 비충돌이라 끼이지는 않아도 모델이 블록에 파묻혀 보이고, 갇힘 폴백이 3초 뒤에야 꺼낸다. **소환하자마자 3초간 벽에 박혀 있는 건 첫인상으로 최악이다.**

그래서 소유자를 중심으로 각도를 돌려가며(뒤 → 좌우 → 앞) 빈 자리를 찾고, 발치와 머리 높이를 둘 다 본다 — 한 칸만 보면 반쯤 파묻힌다. 전부 막혔으면 **소유자가 서 있는 자리**를 쓴다. 사람이 서 있으니 반드시 비어 있다.

여러 마리를 데리고 다닐 때의 배치도 같은 문제를 푼다: 전부 "주인 뒤 한 점"을 노리면 겹쳐 떤다. 슬롯마다 `0° · +35° · -35° · +70° …` 로 벌려 부채꼴로 세운다. 두 계산이 같은 y축 회전을 쓰므로 `Vectors` 로 묶었다 — 한쪽만 고치면 배치가 어긋난다.
...
`setAI(false)` 이므로 바닐라 경로탐색을 쓰지 않는다. 방향 벡터 → 속도 상한 →
**0.4블록 서브스텝으로 분할 검사**(각 지점에서 간이 지형 처리: 앞이 막히고 위가
비었으면 한 칸 오르기, 발밑이 비었으면 최대 3칸 하강 — 그래도 디딜 곳이 없거나
몸이 안 들어가면 그 지점은 버리고 직전까지만 인정) → 위치 갱신. 물·용암·공허
감지 시 즉시 `TELEPORT`.
...
| `true` | `true` | `FLY` | 나는 탑승. 이 종류는 전부 난다 |

**개체별 비행 추첨은 없다.** 예전에는 등급·개체별 `fly-chance` 로 `FLY` 종류의 일부만
날게 했지만, 우클릭 한 번으로 바로 타고 이륙하는 쪽이 더 명확하다는 요청으로 걷어냈다.
`FLY` 종류는 예외 없이 전부 난다.

**생애주기는 탑승을 막지 않는다.** `ride` 가 `NONE` 이 아니면 탈 수 있다 — 진화는 이
판정과 완전히 별개다. 원래는 `ADULT` 여야만 탑승 가능하게 했었는데, "성장은 진화만
하고 탑승은 설정대로 항상 되게 해달라"는 요청으로 그 결합을 풀었다(그 뒤 아기·성체
구분 자체가 없어져 지금은 애초에 물을 필요가 없는 질문이 됐다).

탑승 속도(`ride-speed`)와 비행 상승력(`flight-lift`)은 둘 다 **등급과 무관한
고정 기본값**을 쓴다 — 등급은 표기 전용이라 등급별 기본값이 없다. `ride-speed` 는
`PetCatalog.DEFAULT_RIDE_SPEED`(코드에 박힌 고정값)가, `flight-lift` 는
`config.yml` 의 전역값이 기본값이다. `pets/*.yml` 에 각각을 적으면 그 펫만 다른
값을 쓴다("이 펫만 다르게" 오버라이드를 `PetType` 에 나란히 두는 게 찾기 쉽다).
`flight-lift`·`flight-speed` 는 음수를 "설정 안 함" 신호로 쓴다 — 0으로 접으면
"명시적으로 0"과 "안 적음"을 구분할 수 없기 때문이다.

`seat-offset`(탑승 좌석 높이)도 같은 오버라이드 자리를 쓰지만 **신호로 음수를
못 쓴다** — 좌석을 아래로 내리는 것 자체가 음수 값이라, 음수를 "설정 안 함"으로
겹치면 그 값을 아예 표현할 수 없다. 대신 `Double.NaN` 을 "설정 안 함" 신호로 쓴다.
실제 적용은 `InteractionListener.tryRide` 가 마운트를 스폰할 위치의 y 에 그대로
더하는 것으로 끝난다 — `RideController` 의 이동 로직(`tryMove`)은 전부 상대
이동이라, 스폰 시점에 높이를 한 번만 맞추면 이후 모든 이동이 그 높이를 그대로
유지한다.
...
| 항목 | 설계 |
| --- | --- |
| 자격 | `flying: true` 인 종류는 전부 난다 — 개체별 추첨 없음 |
| 속도 | 겉날개+폭죽보다 느리게 — 원작의 밸런스 의도 |
| 연료 | 없음. 무한 비행 |
| 상승력 | `flight-lift` (기본 0.5, 펫별로 덮어쓸 수 있음) |
...
public final class PetData {
    ...
    private LifeStage stage;       // NORMAL 또는 PIG — 아기·성체 구분은 없다
    private int growth;
...
pets:
  <petId>:
    ...
    stage: NORMAL
    growth: 42
    growth-stage: 2        # next-stage 를 여러 번 탔으면 여러 번 오를 수 있다
...
- **다른 백엔드가 필요해지면** `PetRepository` 인터페이스만 새로 구현하면 된다. SQLite로 가더라도 셰이딩 대신 **Paper 라이브러리 로더**를 쓴다
...
**등급은 표기 전용이다.** 이동속도·탑승속도 같은 실제 능력치에는 관여하지 않는다 —
등급이 실력 차이가 아니라 이름표 하나로만 쓰이도록 요청받아, 등급별 배율을 걷어냈다.
서열(`atLeast`)은 `min-rarity: A` 같은 문턱 판정에서만 의미를 갖는다. 비행 여부도
등급이 아니라 펫 종류의 `flying:` 설정으로만 정해진다.

등급과 표시 이름을 나눈 것은 여전히 유효하다:

| | 어디에 | 왜 |
| --- | --- | --- |
| `Rarity` | 코드 (enum) | 서열이자 식별자다. `min-rarity: A` 같은 설정이 이름으로 비교하고, `atLeast` 가 선언 순서에 기댄다 |
| `RarityStats` | `rarity.yml` | 표시 이름뿐이다. GUI·알림에 어떻게 부를지는 서버마다 다를 수 있다 |

표시 이름은 로드 시점에 **`PetType` 안으로 박아 넣는다.** 표를 들고 다니게 하면 GUI·알림 전부에 표를 넘겨야 한다. 리로드하면 어차피 펫 정의를 통째로 다시 만들므로 값이 굳어 있어도 문제없다.

빠진 등급은 내장 기본값(위 표)으로 채운다 — 언어 파일과 같은 원칙이다.
...
> 들어냈다. **등급별 이동속도·탑승속도 배율도 같은 이유로 걷어냈다** — 등급은 이름표일
> 뿐이라는 요청이다. 펫 종류의 개성은 이제 모델·탑승 속도(`pets/*.yml` 의 `ride-speed`
> 등)로만 표현한다.
...
### 화면에 나가는 것은 전부 언어 파일로

채팅 메시지만이 아니다. **GUI 아이템 이름·로어·인벤토리 제목·명령어 사용법**까지 포함한다.

한동안 GUI 문구가 코드에 박혀 있었다. `language: en_us` 로 바꾸면 채팅은 영어가 되는데 보관함은 한국어로 남았다 — **반쯤 번역된 화면이 아예 번역이 없는 것보다 나쁘다.** 쓰는 사람은 "이 플러그인은 영어를 지원한다"고 믿고 켰는데 절반이 안 되는 셈이다.

생애주기 이름도 마찬가지다. `LifeStage.name()` 을 그대로 쓰면 한국어 화면에 `NORMAL` 이 튀어나온다. `stage.<이름 소문자>` 키로 찾는다.

시간 표기(`time.seconds` 등)는 언어마다 형태가 달라서 문구 자체를 번역 대상으로 둔다. 영어의 `"%s%s"` 는 자리 바로 뒤에 단위가 붙는 형태인데, 치환기가 `%s%` 만 자리로 읽고 뒤의 `s` 를 남겨서 `30s` 가 된다.

### 치환은 한 번에 훑는다

`%name%` 같은 자리를 채울 때 **자리마다 `replace` 를 반복하지 않는다.** 그러면 앞서 넣은 값 안의 `%...%` 가 뒤 차례에 다시 치환된다. 값 중에는 **플레이어가 정한 것**도 있다 — 펫 이름을 `%rarity%` 로 지어두면 그 자리에 등급이 박힌다. 지금은 흉내 내기에 그치지만, 이런 건 나중에 더 나쁜 형태로 돌아온다.

두 가지를 더 지킨다:

- **모르는 자리는 원문 그대로 남긴다.** 번역에서 이름을 잘못 적었을 때 빈칸이 아니라 `%틀린이름%` 이 보여야 눈에 띈다
- **값은 리터럴로 넣는다**(`Matcher.quoteReplacement`). `$1` 이나 백슬래시가 든 펫 이름이 그룹 참조로 해석되면 안 된다

---

## 전체 알림

희귀 펫을 얻거나 진화하면 서버 전체에 알린다. **기본 문턱은 A등급**이다.

문턱을 두는 이유는 단순하다. 아무 펫이나 알리면 5명 서버에서도 채팅이 밀리고, 그러면 관리자가 기능을 통째로 끈다. 등급(`min-rarity`)과 성장 단계(`min-growth-stage`) 두 조건을 **모두** 넘겨야 나간다.

진화 알림은 **두 경로 모두**에서 나가야 한다:

| 경로 | 어디서 |
| --- | --- |
| 먹여서 진화 | `InteractionListener.feed` 의 `STAGE_UP` |
| 시간이 흘러 진화 | `GrowthCatchUp.announce` |

후자를 빠뜨리면 "가만히 뒀더니 조용히 진화했다"가 된다. 성장도가 시간 경과로도 오르는(꺼내둔 동안만이지만) 설계라 이 경로가 오히려 더 흔하다.

**주인에게는 방송 문턱과 무관하게 알린다.** 방송은 A등급부터 나가는데 그것만 두면 D등급 펫은 정말로 조용히 진화한다. 방송의 목적은 자랑이고 개인 알림의 목적은 "네 펫이 달라졌다"라 문턱이 같을 이유가 없다.

### 성장은 계산이지만 진화는 사건이다

성장도는 `updated_at` 기준 지연 계산이라 읽는 순간 저절로 맞는다. 그런데 **진화(종류 전이)는 누가 확인해 줘야 일어난다.** 확인하는 자리가 틱 루프뿐이었고 틱은 소환된 펫만 돈다. 결과:

- 보관함에 넣어둔 펫은 성장도가 상한에 붙은 채 굳었다. 상한에 붙으면 `refresh` 가 값을 바꾸지 않으니 **꺼내서 틱을 태워도 영원히 그대로였다** (틱이 "값이 바뀌었을 때만" 확인했다)
- 꺼내둔 채로 접속하지 않은 동안 자란 펫도 마찬가지다

`GrowthCatchUp` 이 그 확인을 맡는다. 펫을 **읽는 자리마다** 지난다 — 접속 직후, 보관함을 열 때, `/pet list`, 틱 루프. 넷 다 이미 목록을 훑는 자리라 새 주기 작업이 생기지 않는다. (지연 계산을 택한 이유가 그거였다)

`GrowthService.catchUp` 은 `refresh` + `promoteIfGrown` 을 **짝으로 묶는다.** 나눠 부르면 위의 "값이 안 바뀌니 확인도 안 한다"가 다시 자란다.
...
개별 플레이어가 Bedrock 인지 구분하지 않는다. 예전에는 Floodgate 의
`FloodgateApi.isFloodgatePlayer(uuid)` 로 구분해 비행 이륙 확인("3초 안에 한 번 더
우클릭")을 Bedrock 플레이어에게만 건너뛰게 했다 — 터치 조작에서 두 번째 우클릭을
맞히기가 훨씬 어려워서였다. 그 확인 절차 자체를 없애면서(자바·Bedrock 모두 우클릭
한 번이면 탑승·이륙한다) 개별 구분이 더는 필요 없어 걷어냈다.
...
├─ rarity.yml      등급별 수치 (속도 · 색)
├─ pets/*.yml       펫 종류 정의 (egg · baby_phantom · phantom_normal · phantom_ender · pig 예시 제공)
...
`MovementController.followTarget`·`step` 의 할당도 남겨뒀다. 틱마다 `Location`·`Vector` 를 몇 개씩 만든다. 없애려면 목표 지점 계산을 yaw 삼각함수와 버퍼로 다시 쓰고 부채꼴 각도까지 손으로 돌려야 하는데, **서버 없이는 추종이 여전히 자연스러운지 확인할 수 없다.** 5명 규모에서 아끼는 양(초당 수백 개의 짧은 수명 객체 — JVM 의 신세대가 사실상 공짜로 처리하는 크기)보다 "펫이 이상하게 걷는다"가 훨씬 비싸다. 실측이 가능해지면 그때 판단한다.
...
- [ ] 걷는 펫을 명령으로 공중에 순간이동시키면(예: `/tp`), 스스로 땅까지 떨어지는가
- [ ] **나는 펫을 데리고 주인이 비행하면, 펫이 따라 올라오는가** (땅에서 쫓아오다 3초 뒤
      순간이동하는 게 아니라 실제로 날아서 따라와야 한다)
- [ ] **나는 펫이 지형을 기어다니지 않고 높이를 유지하는가** — 주인이 계단이나 언덕을
      오르내릴 때 땅에 붙지 않고 자연스럽게 따라오는가
- [ ] **나는 펫이 천장이 낮은 실내에서 블록에 끼지 않는가** (막히면 그 앞에 멈췄다가
      갇힘 판정으로 순간이동하는 것이 정상)
...
개발을 막는 질문은 없다. 남은 것은 **실제로 돌려봐야 정해지는** 것들이다.

| # | 질문 | 지금 상태 |
| --- | --- | --- |
| **Q3** | 라이선스 | 정하지 않았다. `LICENSE` 파일이 없으면 기본이 "모든 권리 보유"라 남이 쓰거나 고칠 수 없다. 참고한 BetterModel 과 betterpets-paper 는 둘 다 MIT |

해소된 질문: JDK 버전(Java 25 — 바이트코드로 확인), 서버 규모(5명), 저장소(YAML), 경제 연동(보류), NPC(제거), 모델 에셋(직접 제작), 비행 조작(`ArmorStand` + `Input`), 등급 수치의 위치(`rarity.yml`), 능력 시스템(제거), 등급별 이동·탑승속도 배율(제거 — 등급은 표기 전용).
