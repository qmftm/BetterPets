...
탑승 속도(`ride-speed`)와 비행 상승력(`flight-lift`)은 같은 자리 방식이다 —
`ride-speed` 는 `rarity.yml` 의 등급별 값이 기본값이고, `flight-lift` 는
`config.yml` 의 전역값이 기본값이다. `pets/*.yml` 에 각각을 적으면 그 펫만 다른
값을 쓴다("이 펫만 다르게" 오버라이드를 `PetType` 에 나란히 두는 게 찾기 쉽다).
`flight-lift` 는 음수를 "설정 안 함" 신호로 쓴다 — `rideSpeed` 처럼 등급 기본 표가
없어서, 0으로 접으면 "명시적으로 0"과 "안 적음"을 구분할 수 없기 때문이다.
