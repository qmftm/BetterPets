...
| 남이 펫을 우클릭해 아이템을 잃음 | 캐리어(`Allay`)·마운트(`ArmorStand`) 모두 <b>바닐라 상호작용이 손에 든 것을 받아 간다</b>. 상호작용은 소유자 확인보다 <b>먼저</b> 취소하고, 마운트는 슬롯을 통째로 잠근다 | ✅ 우리가 찾은 것 — 오프핸드와 "남의 펫"이 실제 구멍이었다 |
| 펫이 채굴·건축을 방해 | 우클릭 동작이 없는 펫은 히트박스를 아예 작게 | ❌ 위와 같은 이유. 대신 캐리어를 **`Allay`**(0.35×0.6)로 골라 애초에 작게 뒀다 |
