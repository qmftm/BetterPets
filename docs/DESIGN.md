...
> ⚠️ **종류가 바뀌면 모델을 다시 붙여야 한다.** `ActivePet` 은 소환 시점의 `PetType` 을 붙들고 있어서 `typeId` 만 갈아도 예전 모델과 예전 애니메이션 이름을 계속 쓴다. 과급식 변신과 성장 단계 진화 두 경로 모두 `PetService.refreshIfTypeChanged` 로 마무리한다. 이걸 빠뜨려서 실제로 진화가 화면에 안 나타나는 버그가 있었다.
