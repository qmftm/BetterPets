...
- **주석은 한국어로 쓴다.** 설명이 아니라 **판단의 근거**를 적는다. "이 값을 캐시한다"가
  아니라 "매 틱 도는 자리이기 때문에 캐시한다"처럼 이유가 드러나야 한다.
- **새로 쓰는 주석은 엠대시(—) 대신 콜론이나 접속사를 쓰고, 문장을 완결된 서술어로
  끝맺는다.** "박아넣다"·"치우다"·"얹다" 같은 구어체 비유보다 "명시한다"·"제거한다"·
  "추가한다"처럼 뜻이 분명한 어휘를 우선한다. 이 규칙은 앞으로 새로 작성하는 주석에
  적용하고, 기존 주석 수백 곳을 일괄로 고치지는 않는다.
...
- **클릭 핸들러에서 `setCancelled(true)` 를 가장 먼저 부른다.** 로직 뒤로 미루면
  중간에 예외나 조기 return 이 걸리는 순간 그게 그대로 구멍이 된다. GUI 에서는 아이템
  복제가 되고, **엔티티 상호작용에서는 아이템이 사라진다** — 캐리어는 `Allay`(나는 펫)
  또는 `Rabbit`(걷는 펫), 마운트는 `ArmorStand` 라서 취소하지 않으면 <b>바닐라 동작이
  손에 든 것을 받아 간다.</b>
  오프핸드와 "남의 펫"이 실제로 그 구멍이었다. 소유자 확인·손 확인은 전부 취소 뒤에 한다.
...
- **성장이 일어난 뒤에는 반드시 `PetService.refreshAfterGrowth` 를 부른다.**
  ...
  급여·시간 경과·`/quantumpets growth` 세 경로 모두 이 마무리가 필요하다.
  같은 이유로 **알림에도 `pet.type()` 을 쓰면 안 된다** — 진화 전 이름을 부르게 된다.
- **설정을 읽어 들고 있는 쪽은 `/quantumpets reload` 때 다시 밀어 넣어야 한다.**
  ...
  **이미 소환된 펫은 일부러 손대지 않는다.** `ActivePet` 은 소환 시점의 `PetType` 을
  붙들고 이동 속도를 굳혀두므로, 리로드해도 나와 있는 펫은 예전 정의로 돈다.
  전부 재소환하면 반짝일 뿐 아니라 **설정 오타 하나로 전원의 펫이 사라진다**(새 모델
  id 가 없으면 재소환이 실패한다). 대신 몇 마리가 그 상태인지 리로드 응답에 적는다.
...
- **사용자에게 보이는 문자열을 코드에 적지 않는다.** 채팅뿐 아니라 **GUI 아이템 이름·로어·
  ...
  화면이 아예 번역이 없는 것보다 나쁘다. `/quantumpets debug` 의 진단 줄도 같은 이유로
  뒤늦게 옮겼다.
...
`PlaceholderAPI` · `Skript` 는 상속이 필요해 `provided` 의존을 건다. 넷 다 **실패하면
조용히 꺼진다.** 연동이 안 되는 것보다 그 때문에 펫 시스템이 멈추는 게 훨씬 나쁘다.
