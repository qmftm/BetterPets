...
- **사용자에게 보이는 문자열을 코드에 적지 않는다.** 채팅뿐 아니라 **GUI 아이템 이름·로어·
  ...
  **선은 "보통 플레이어가 볼 수 있는가"로 긋는다.** `PetCatalog.problems` 의 설정 오류
  문구는 예외로 두고 코드에 남긴다. 이유는 셋이다 — (1) `items.yml/eggs/x: gives...`
  처럼 **번역되지 않는 파일 경로와 키가 문장 안에 박혀 있어서**, 그 주위만 번역하면
  결국 같은 "반쯤 번역된 줄"이 된다. (2) 스물몇 개의 구조화된 템플릿을 번역자가
  동기화해야 하고, 자리 이름 하나가 어긋나면 **진단문이 조용히 엉뚱한 말을 한다**.
  (3) 틀을 이루는 문구(`admin.reloaded-with-problems`)는 이미 번역돼 있다 — 안쪽은
  진단 내용이지 화면 문구가 아니다.
- **성장도를 반영했다고 성체가 되지는 않는다.** `refresh` 는 값만 맞추고,
  전이는 `promoteIfGrown` 이 한다. 게다가 **상한에 붙으면 `refresh` 가 아무것도 안 바꾸므로**
  "값이 바뀌었을 때만 확인"으로 이으면 딱 그 펫들이 영원히 아기로 남는다. 둘은
  `GrowthService.catchUp` 으로 묶여 있고, 펫을 읽는 자리(접속·보관함·`/pet list`·틱)는
  모두 `GrowthCatchUp` 을 지나야 한다.
